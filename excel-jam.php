...
<?php
			if ($cat == 'all') {
                $start = date('Y-m-d 00:01:00', strtotime($q));
                $end = date('Y-m-d 00:00:00', strtotime($q."+1 day"));
                $sql =  "select * from data where parameter in ('".$prm."') and cerobong_id = '$cerobong' and waktu between '$start' and '$end' order  by parameter, waktu";
                //echo $sql;
?>
				<br>
<?php
...
<?php
				...
				while ($data2 = mysqli_fetch_array($query2, MYSQLI_ASSOC)) {
                    $value += $data2['value'];
					$no++;
                    $menit = date('ii', strtotime($data2['waktu']));
					echo '
						<tr>
							<td>'.$no.'</td>
                    ...
                    if ($menit == '00') {
                        $ratarata = $value/12;
                        echo '
                                <td style="border-top: none;">'.$ratarata.'</td>
						    </tr>
                        ';
                        $value = 0;
                    }else{
