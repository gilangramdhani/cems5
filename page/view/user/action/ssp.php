...
if (isset($_SESSION['id'])) {
	...
	$sql = "SELECT {$primarykey} ".implode(',', $columns)." FROM {$from} WHERE user_role <> 'Administrator'";
	...
	while ($row = $data->fetch_assoc()) {
		$fields = array();
		for ($i = 0; $i < count($columns); $i++) {
			$fields[0] = $row['user_id'];
			$fields[1] = $row['user_email'];
			$fields[2] = $row['user_id'];
			$fields[3] = $row['user_full'];
			$fields[4] = $row['user_role'];
			$fields[5] = $row['user_notif'];
			$fields[6] = $row['user_id'];
		}
