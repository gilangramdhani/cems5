...
if (isset($_SESSION['id'])) {
	...
	if ($response['status'] == 'verified') {
		...
		if ($action == 'download') {
			...
			if ($cat <> 'all') {
				$query = mysqli_query($con, "select * from data where parameter = '$cat' and cerobong_id = '$cerobong' and date(waktu) between date('$fromdate') and date('$todate') order by waktu desc");
			}
			if ($cat == 'all') {
				$query = mysqli_query($con, "select sum(case when parameter = 'NOx' THEN value END) NOx, sum(case when parameter = 'SO2' THEN value END) SO2, sum(case when parameter = 'PM' THEN value END) PM, sum(case when parameter = 'CO2' THEN value END) CO2, sum(case when parameter = 'Hg' THEN value END) Hg, sum(case when parameter = 'O2' THEN value END) O2, waktu, laju_alir, status_gas, status_partikulat, status, fuel, `load`, cerobong_id, modified_at from data where cerobong_id = '$cerobong' and date(waktu) between date('$fromdate') and date('$todate') group by DATE_FORMAT(waktu, '%Y-%m-%d %H:%i') order by DATE_FORMAT(waktu, '%Y-%m-%d %H:%i') desc;");
			}
			$dir = '../../../../download/';
			...
			';
			$html .= '<hr><p class="title"><strong>Laporan</strong><br><u>Grafik rata - rata data pada tanggal '.tanggal($fromdate).' sampai tanggal '.tanggal($todate).'</u></p>';
			$html .= '<img src="'.$uri.'"><br><br><page_break>';
			$html .= '<p align="center"><u>Detail data pada tanggal '.tanggal($fromdate).' sampai tanggal '.tanggal($todate).'</u></p>';
			if ($cat <> 'all') {
				$html .= '
					<table class="demo">
						<thead>
