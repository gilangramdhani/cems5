...
if (isset($_SESSION['id'])) {
	...
	if ($cat <> 'all') {
		$sql = "SELECT {$primarykey} ".implode(',', $columns)." FROM {$from} WHERE a.parameter = '$cat' and a.cerobong = '$cerobong' and date(a.tanggal) between date('$fromdate') and date('$todate')";
	}
	if ($cat == 'all') {
		$sql = "SELECT {$primarykey} ".implode(',', $columns)." FROM {$from} WHERE a.parameter in ('".$prm."') and a.cerobong = '$cerobong' and date(a.tanggal) between date('$fromdate') and date('$todate')";
	}
