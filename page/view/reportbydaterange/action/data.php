...
if (isset($_SESSION['id'])) {
	...
	if ($cat <> 'all') {
		$query = mysqli_query($con, "select avg(value) as y, date(waktu) as x from data where parameter = '$cat' and cerobong_id = '$cerobong' and date_sub(waktu, interval 1 day) and date(waktu) between date('$fromdate') and date('$todate') group by date(waktu) order by date(waktu) asc");
	...
	if ($cat == 'all') {
		...
		while ($parameterallData = mysqli_fetch_array($parameterallQuery, MYSQLI_ASSOC)) {
			...
			$query = mysqli_query($con, "select avg(value) as y, date(waktu) as x from data where parameter = '$parameterall' and cerobong_id = '$cerobong' and date_sub(waktu, interval 1 day) and date(waktu) between date('$fromdate') and date('$todate') group by date(waktu) order by date(waktu) asc");
			if (mysqli_num_rows($query) > 0) {
		...
		$waktuQuery = mysqli_query($con, "select avg(value) as y, date(waktu) as x from data where parameter = '$parameterwaktu' and cerobong_id = '$cerobong' and date_sub(waktu, interval 1 day) and date(waktu) between date('$fromdate') and date('$todate') group by date(waktu) order by date(waktu) asc");
