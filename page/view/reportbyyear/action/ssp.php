...
if (isset($_SESSION['id'])) {
	...
	if ($cat <> 'all') {
		$sql = "SELECT {$primarykey} ".implode(',', $columns)." FROM {$from} WHERE a.parameter = '$cat' and a.cerobong = '$cerobong' and year(a.tanggal) = year('$q')";
	}
	if ($cat == 'all') {
		$sql = "SELECT {$primarykey} ".implode(',', $columns)." FROM {$from} WHERE a.parameter in ('".$prm."') and a.cerobong = '$cerobong' and year(a.tanggal) = year('$q')";
	}
