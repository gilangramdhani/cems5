...
if (isset($_SESSION['id'])) {
	...
	if ($cat <> 'all') {
		$sql = "SELECT {$primarykey} ".implode(',', $columns)." FROM {$from} WHERE d.parameter = '$cat' and d.cerobong_id = '$cerobong' and year(d.waktu) = year('$q')";
	}
	if ($cat == 'all') {
		$sql = "SELECT {$primarykey} ".implode(',', $columns)." FROM {$from} WHERE d.parameter in ('".$prm."') and d.cerobong_id = '$cerobong' and year(d.waktu) = year('$q')";
	}
